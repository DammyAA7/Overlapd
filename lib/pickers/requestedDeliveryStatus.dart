...
class RequestedDeliveryStatus extends StatefulWidget {
  ...
  @override
  State<RequestedDeliveryStatus> createState() =>
      _RequestedDeliveryStatusState();
}
          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: solidButton(
                    context, 'Upload Receipt', () => _captureReceipt(), true),
              ),
              Row(
                children: [
                  Expanded(
                      ...
                      child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        child:
                            numberInputBox('Enter Total', true, false, total),
                      )),
                  Expanded(
                      flex: 3,
                      child: Padding(
                        ...
                        child: solidButton(context, 'Capture Payment', () {
                          showDialog(
                            ...
                            builder: (BuildContext dialogContext) =>
                              ...
                              content:
                                  Text('€${total.getText()} paid at checkout?'),
                              actions: <Widget>[
                                TextButton(
              ...
              Padding(
                padding: const EdgeInsets.all(8.0),
                child:
                    Text('Total Shopped: €${totalShopped.toStringAsFixed(2)}'),
              ),
              Expanded(
                            ...
                            onChanged: (bool? value) {
                              ...
                              if (value!) {
                                setState(() {
                                  ...
                                  itemsForDelivery[index]['unavailable'] =
                                      !value!;
                                  totalShopped += (item['product']['price'] *
                                      item['quantity']);
                                });
                              }
                            },
  ...
  void captureAmount() async {
    ...
    if (orderDocument.exists && orderDocument.data() != null) {
      ...
      String paymentIntentId =
          data['payment id']; // Assuming field name is 'paymentIntentId'
      ...
      double deliveryFees =
          double.parse(deliveryFeesWithEuro.replaceAll('€', ''));

      double serviceFees = double.parse(
          (double.parse(total.getText()) * 0.11).toStringAsFixed(2));
  ...
  Future<void> fetchOrderDetails() async {
    ...
    if (orderSnapshot.exists) {
      ...
      setState(() {
        ...
        totalPaid =
            double.tryParse(data['Item Total'].replaceAll('€', '')) ?? 0.0;
