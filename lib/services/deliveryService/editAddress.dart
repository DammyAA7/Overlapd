...
class _EditAddressState extends State<EditAddress> {
  ...
  void initState() {
    ...
    streetAddressController =
        TextEditingController(text: widget.addressDetails?['Street Address']);
    localityController =
        TextEditingController(text: widget.addressDetails?['Locality']);
    countyController =
        TextEditingController(text: widget.addressDetails?['County']);
    postalCodeController =
        TextEditingController(text: widget.addressDetails?['Postal Code']);
    isDefault = widget.addressDetails?['Default'] ?? false;
  }

  void checkForModifications() {
    setState(() {
      isModified = streetAddressController.text !=
              widget.addressDetails?['Street Address'] ||
          ...
          child: Column(
            ...
            children: [
              ...
              addressInputBox(
                  ...
                  TextInputType.streetAddress, (newValue) {
                streetAddressController.text = newValue;
                checkForModifications();
              }),
              const Text("Locality"),
              addressInputBox('Locality', true, false, localityController.text,
                  TextInputType.streetAddress, (newValue) {
                localityController.text = newValue;
                checkForModifications();
              }),
              const Text("County"),
              addressInputBox('County', true, false, countyController.text,
                  TextInputType.streetAddress, (newValue) {
          ...
          () async {
            if (isModified) {
              ...
              List<dynamic> addressBook =
                  List.from(userDoc.data()?['Address Book'] ?? []);

              // Find the index of the address being edited
              int indexToUpdate = addressBook.indexWhere((address) =>
                  address['Full Address'] ==
                  widget.addressDetails?['Full Address']);
              ...
              if (indexToUpdate != -1) {
                ...
                addressBook[indexToUpdate] = {
                  ...
                  'Default': isDefault,
                  'Full Address':
                      "${streetAddressController.text}, ${localityController.text}, ${postalCodeController.text}, ${countyController.text}",
