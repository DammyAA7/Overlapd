...
class _AddDeliveryAddressState extends State<AddDeliveryAddress> {
                    ...
                    Padding(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () async {
                          ...
                          double distanceInMeters = calculateLatLngDistance(
                          ...
                          if (distanceInMeters > 5000) {
                            ...
                            AddressComponents? formattedAddress =
                                await getAddressDetailsFromCoordinates(
                                    currentLocation!.latitude,
                                    currentLocation!.longitude);
                            setState(() {
                    ...
                    Expanded(
                      child: placePredictions.isNotEmpty
                          ...
                          : predictions == ''
                              ...
                              : const Center(
                                  child: Text(
                                      'No results found, You address may be incorrect or outside our deliverable area'),
                                ),
                    ),
                  ],
                )
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: solidButton(context, "Save Address", () async {
                          setState(() {
                            fullAddress =
                                '${fullStreetAddress!}, ${locality!}, ${postalCode!}, ${county!}';
                          });
                          ...
                          if (coordinates != null) {
                            ...
                            if (distanceInMeters > 5000) {
                              ...
                              await setDefaultAddress(
                                  _UID, addressBook, defaultAddress);
                              setAddress = null;
                              searchText.clear();
                              predictions = '';
                              streetAddress = '';
                            ...
                            showDialog(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  ...
                                  content: Text(
                                      "No results found for the given address. Please correct the address and try again."),
                                  actions: <Widget>[
                                    TextButton(
                                      child: Text('OK'),
                                      onPressed: () {
  ...
  Future<void> setDefaultAddress(String userId, Map<String, dynamic> newAddress,
    ...
    if (docSnapshot.exists &&
        docSnapshot.data()!.containsKey('Address Book') &&
        docSnapshot.data()!['Address Book'].isNotEmpty) {
      ...
      bool addressExists = addressBook.any((address) {
        String existingAddressNormalized =
            (Map<String, dynamic>.from(address)['Full Address'] as String)
                .toLowerCase() // Make lowercase
                .replaceAll(',', '') // Remove commas
      ...
      if (defaultAddress) {
        addressBook = addressBook.map((address) {
          final Map<String, dynamic> modifiedAddress =
              Map<String, dynamic>.from(address);
          modifiedAddress['Default'] =
              false; // Reset default flag for all other addresses
  ...
  Future<Map<String, dynamic>?> fetchCoordinatesFromAddress(
