...
class _CheckoutState extends State<Checkout> {
  ...
  void filterScheduleDeliveryTimes() {
    ...
    if (now.isAfter(DateTime(now.year, now.month, now.day, 17, 0, 0)) &&
            now.isBefore(DateTime(now.year, now.month, now.day, 23, 59, 59)) ||
        now.isAfter(DateTime(now.year, now.month, now.day, 0, 0, 0)) &&
            now.isBefore(DateTime(now.year, now.month, now.day, 7, 59, 59))) {
      ...
      now.hour - 7 < 10
          ? newScheduleDeliveryTimes
              .add('Slot (${now.hour - 11}pm - ${now.hour - 7}pm)')
          : null;
      ...
      now.hour - 3 < 10
          ? newScheduleDeliveryTimes
              .add('Slot (${now.hour - 7}pm - ${now.hour - 3}pm)')
          : null;
      ...
      now.hour + 1 < 10
          ? newScheduleDeliveryTimes
              .add('Slot (${now.hour - 3}pm - ${now.hour + 1}pm)')
          : null;
    }
    setState(() {
        ...
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              now.hour + 1 > 20 || now.hour + 1 < 8
                  ? const Text('All stores are currently closed')
                                        ...
                                        children: [
                                          Flexible(
                                            child: CheckboxListTile(
                                              title: Text(
                                                "Picker has full discretion over substitutable items",
                                          ...
                                          Flexible(
                                            child: CheckboxListTile(
                                              title: Text(
                                                "Picker will contact you to get your approval",
                                                style: Theme.of(context)
                                                    .textTheme
                                                    .titleLarge,
                                              ),
              ...
              GestureDetector(
                onTap: () => rewardCardUrl == null
                    ? _captureRewardCard()
                    : _viewOrRetakeRewardCard(),
                child: ListTile(
                    contentPadding: EdgeInsets.zero,
                    horizontalTitleGap: 5,
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                            '€${value.calculateServiceFees().toStringAsFixed(2)}')
                      ],
                    ),
                    const Divider(),
                    Row(
                    ...
                    solidButton(context, 'Checkout', () async {
                      await initPayment(
                          (double.parse(value.stripEuroSign(
  ...
  Future<void> initPayment(
      String amount, String email, BuildContext context, Cart value) async {
    try {
      ...
      _checkout('Tesco', value.cart, feeBreakdown[3], feeBreakdown[1],
          feeBreakdown[2], jsonResponse['id']);
    } catch (e) {
      print(e);
    }
  }

  void _checkout(String chosenStore, Map<Product, int> products, String amount,
      String serviceFee, String deliveryFees, String paymentID) async {
    List<Map<String, dynamic>> productListMap =
        context.read<Cart>().toMapList();
    await _service.openDelivery(
        fullAddress!,
        GeoPoint(addressCoordinates!.latitude, addressCoordinates!.longitude),
        chosenStore,
        productListMap,
        amount,
        paymentID,
    ...
    Navigator.of(context)
        .pushReplacement(pageAnimationFromTopToBottom(const Home()));
                  ...
                  child: Column(
                    children: [
                      now.isAfter(
                          ...
                          ? const Text(
                              'Scheduled Deliveries available tomorrow')
                          : const SizedBox.shrink(),
                      Expanded(
                                ...
                                onChanged: (bool? value) {
                                  if (value == true) {
                                    setState(() {
                                      ...
                                      updateChosenScheduleDeliveryTime(
                                          scheduleDeliveryTimes[index]);
                                    });
                                  }
                                },
  ...
  Future<void> loadLastSelectedOrDefaultAddress() async {
    final userDocSnapshot = await _auth.getAccountInfoGet(
        _UID); // Assuming this returns a Future<DocumentSnapshot>
    Map<String, dynamic>? userData = userDocSnapshot.data();
    if (userData != null) {
      if (userData.containsKey('lastAddressSelected') &&
        ...
        setState(() {
          ...
          addressCoordinates = GeoPoint(
              (userData['lastAddressSelected']['Coordinates']['lat']),
              userData['lastAddressSelected']['Coordinates']['lng']);
        });
      } else if (userData.containsKey('Address Book') &&
          userData['Address Book'].isNotEmpty) {
        // Check for a default address in the address book
        final defaultAddress = (userData['Address Book'] as List)
